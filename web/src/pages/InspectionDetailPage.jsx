...
export function InspectionDetailPage() {
  ...
  async function confirmDecision() {
    ...
    try {
      const result = await request(`/inspections/${id}/admin-decision`, { token, method: 'PATCH', body: { decision: pendingDecision, findingIds: selectedFindingIds } });
  ...
  const choosingFinding = ['POTENTIAL_ISSUE', 'PRODUCT_REJECTED'].includes(pendingDecision);
  const presentationStatus = inspection.admin_decision === 'PRODUCT_REJECTED' ? 'PRODUCT_REJECTED' : inspection.admin_decision === 'POTENTIAL_ISSUE' ? 'POTENTIAL_ISSUE' : inspection.state;
  ...
  return <>
    ...
    <div className="detail-grid">
      <section className="panel"><h3>Inspection record</h3><dl><dt>Brand</dt><dd>{inspection.brand_name || 'Not recorded'}</dd><dt>Location</dt><dd>{inspection.location || 'Not recorded'}</dd><dt>Created</dt><dd>{new Date(inspection.created_at + 'Z').toLocaleString()}</dd><dt>Notes</dt><dd>{inspection.notes || 'None'}</dd></dl></section>
