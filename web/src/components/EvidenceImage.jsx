...
export function EvidenceImage({ inspectionId, image, token }) {
  ...
  return <div>{url && <a href={url} target="_blank" rel="noreferrer"><img className="evidence-thumb" src={url} alt={image.original_filename}/></a>}<strong>{image.image_type}</strong><span>{image.original_filename}</span><small>Image quality: {image.quality_state}{image.quality_reason ? ` · ${image.quality_reason}` : ''}{image.quality_override ? ' · Officer chose to use this image anyway.' : ''}</small></div>;
}
