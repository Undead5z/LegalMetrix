...
async function assessDeclarations({ inspectionId, declarations, ocrConfidence = 1 }) {
  ...
  const findings = rules.map(rule => {
    ...
    const datePointers = rule.declaration_field === 'date_declaration' ? declarations.filter(item => fieldsFor.date_declaration.includes(item.field) && item.date_reference_pointer) : [];
    if (rule.applicability === 'DATE_APPLICABLE' && !values.length && datePointers.length) return { ruleId: rule.id, field: rule.declaration_field, observedValue: null, status: 'REVIEW_REQUIRED', confidence: 0, explanation: 'Date information is referenced elsewhere on the package. Capture the top/crimp area.', evidence: datePointers.map(item => ({ declarationId: item.id, sourceImageId: item.sourceImageId, reference: item.date_reference_pointer })), legalReference: rule.legal_reference, ruleVersion: rule.version };
