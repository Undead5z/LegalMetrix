...
function textLines(data, reportNumber) {
  ...
  lines.push('LEGALMETRIX - INSPECTION REPORT', ''); add('Report Number', reportNumber); add('Inspection ID', i.id); add('Inspection Number', i.inspection_number); add('Product', i.product_name); add('Category', i.generic_name || 'Not recorded'); add('Brand', i.brand_name || 'Not recorded'); add('Inspection Date/Time', `${i.created_at} UTC`); add('Officer', `${i.officer_name} (${i.officer_email})`); add('Final Label Compliance Outcome', i.admin_decision ? i.admin_decision.replace(/_/g, ' ') : i.state); add('Product Condition (informational, separate from label compliance)', i.product_condition || 'UNKNOWN'); if (i.product_condition_reason) add('Product Condition Reason', i.product_condition_reason); if (adminDecisionFindings.length) add('Administrator decision basis', adminDecisionFindings.map(finding => `${finding.status} | ${finding.rule_code || 'Rule unavailable'} | ${finding.message}`).join(' || ')); if (i.admin_decider_name) add('Administrator decision by', `${i.admin_decider_name} at ${i.admin_decided_at || '-'}`); add('Location', i.location || 'Not recorded'); lines.push('', 'EXTRACTED DECLARATIONS');
